...
The typical settings for the original \FedAvg{} can be changed in
...
\FedProx{}~\cite{li2020federated} is proposed to
allow for each client to train for a various number of local epochs according to
their computational abilities.
\FedProx{} has been shown to be much more efficient than
\FedAvg{} in heterogeneous settings.
...
\TFF{} by Google is widely used as infrastructure for
its rich features.
